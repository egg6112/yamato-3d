...
### 2.2.1 艦橋ディテール `buildBridgeDetails()`

緻密化のため以下を追加。細物はカテゴリ別に `mergeToMesh()` で 2 メッシュへ結合し
ドローコール増加を抑制（追加 約 2.5 万三角形）。

| 部位 | 内容 |
|------|------|
| 窓枠 | 羅針艦橋ガラス帯の縦桟（前面 14 本・側面各 11 本）＋上下トリム |
| 手すり | 張り出しデッキ縁（円形 r5.05・26 分割）／防空指揮所パラペット上端（r3.32・18 分割）／左右ウィング外縁（コの字） |
| 双眼鏡座 | 防空指揮所に 4 基（ペデスタル＋横置き双眼鏡） |
| ラッタル | 後面昇降はしご 3 段（レール 2 本＋踏桟 0.5m ピッチ） |
| 22 号電探 | 左右舷のラッパ型ホーンアンテナ（円錐＋導波管箱＋ブラケット） |
| 15m 測距儀 | 光学副筒 2 本（r0.16 全長 13.6m）＋前面スリットフード |
| 張線 | 前檣バックステー 2 本＋信号旗ハリヤード 2 本 |
